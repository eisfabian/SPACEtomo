...
# Collect montage on new grid and save it
def collectWGMap(grid_name, map_dir, mic_params, montage_overlap, model):
    ...
    if len(grid_name) > 6:
         sem.Echo("WARNING: Grid name is longer than 6 characters and will be truncated for labels in the Navigator.")
...
def drawNavPoints(grid_name, map_dir, mic_params, model, include_broken=False, distance_threshold=5):
    # Read bboxes
    with open(os.path.join(map_dir, grid_name + "_boxes.json"), "r") as f:
        bboxes = json.load(f, object_hook=space_ext.revertArray)

    # Load map to draw points
    map_id = sem.NavIndexWithNote(grid_name + ".mrc")
    sem.LoadOtherMap(int(map_id))
    ...
    if len(PL_nav_items) > 0 or len(FL_nav_items) > 0:
        sem.Echo("WARNING: Already found navigator items for detected lamellae. Skipping intermediate mag images.")
...
def findOffset(grid_name, map_dir, lamella_ids, initial_bboxes, mic_params, model, keep_threshold=0.7, distance_threshold=5, external=False):
    ...
    if len(IM_map_files) > 0:
         sem.Echo("WARNING: Intermediate mag images were already recorded. Skipping...")
    ...
    for l, lamella_id in enumerate(lamella_ids):
        sem.Echo("Moving to lamella " + str(l + 1) + "...")
...
def saveAsTargets(buffer, map_dir, map_name, model, mic_params):
    ...
    if len(tgts_files) > 0:
         sem.Echo("WARNING: Targets file for " + map_name + " already exists. Skipping target setup!")
         return
...
class MicParams:
    ...
    def export(self, map_dir):
        with open(os.path.join(map_dir, "mic_params.json"), "w+") as f:
             json.dump(vars(self), f, indent=4, default=space_ext.convertArray)
        sem.Echo("NOTE: Saved microscope parameters at " + os.path.join(map_dir, "mic_params.json"))
...
def loadGrid(grid_slot, grid_name, load_grid):
    ...
    if DEVICE == "cuda":
         sem.Echo("NOTE: GPU will be used.")
    else:
         sem.Echo("NOTE: No GPU found. CPU will be used.")
